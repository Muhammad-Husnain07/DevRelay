...
async function createWebhookEvents(workspace) {
  ...
  for (const eventData of events) {
    await WebhookEvent.create({
      ...
      payload: { user_id: Math.floor(Math.random() * 1000), event: eventData.type }
    });
  }
}
...
async function createScheduledJobs(workspace) {
  ...
  const jobs = [
    { name: 'Daily Backup', cron: '0 2 * * *', description: 'Run daily backup at 2am' },
    { name: 'Weekly Report', cron: '0 8 * * 1', description: 'Send weekly report every Monday' },
    { name: 'Every 5 Minutes', cron: '*/5 * * * *', description: 'Heartbeat check every 5 minutes' }
  ];
...
async function getStats() {
  const [users, workspaces, webhookEndpoints, inboundWebhooks, webhookEvents, jobs, scheduledJobs, emailTemplates, gatewayRoutes, consumers, alertRules] = await Promise.all([
    User.countDocuments(),
  ...
  return { users, workspaces, webhookEndpoints, inboundWebhooks, webhookEvents, jobs, scheduledJobs, emailTemplates, gatewayRoutes, consumers, alertRules };
}
